...
- Plots Beam and Diffuse components from both datasets on the same graph.
...
############################## File Paths #####################################
bsrn_file_path = Path(r"C:\Users\arthurg\OneDrive - NTNU\Workspace\Data\Irradiance\NYA\NYA_radiation_2025-03.tab")
glob_data_path = Path(r"C:\Users\arthurg\OneDrive - NTNU\Workspace\Data\GLOB\B_and_D_Estimations_NYA")
...
while current_date <= end_date:
    ...
    glob_filename = glob_data_path / f"best_estimations_{file_date_str}_ERBS.csv"
    ...
    # Load the BSRN data
    try:
        bsrn_data = pd.read_csv(bsrn_file_path, sep='\t', skiprows=24, parse_dates=['Date/Time'], index_col='Date/Time')
    ...
    bsrn_data = bsrn_data[(bsrn_data.index.normalize() == pd.Timestamp(file_date_str)) &
                          (bsrn_data.index.hour >= 9) &
                          (bsrn_data.index.hour < 16)]
