...
import pvlib
import xarray as xr
from config_path import B_D_DATA_PATH, ALL_PLOT_PATH, GLOB_DATA_PATH, NYA_DATA_PATH
...
ds = xr.open_dataset(GLOB_DATA_PATH / f"GLOB_data_10min_{year}.nc")
...
for date in dates:
    ...
    ax2.annotate(beam_bsrn.index[0].strftime('%Y-%m-%d'), 
             xy=(0.90, -0.15), xycoords='axes fraction')
    plt.xticks()
    plt.yticks()
    ...
    output_file = ALL_PLOT_PATH / "Beam_and_Diffuse" / f"{year}" / \
        f'Beam_Diffuse_{file_date_str}_{pyrano_nr}pyrano_{method}.png'
...
import numpy as np
import pvlib
from config_path import DATA_PATH, B_D_DATA_PATH, ALL_PLOT_PATH, GLOB_DATA_PATH, NYA_DATA_PATH
...
B_D_estimations_datafile = B_D_DATA_PATH / \
    f"{year}_estimation_beam_diffuse_{f}min_{method}_{pyrano_nr}pyrano.csv"

output_plot_path = ALL_PLOT_PATH / "Beam_and_Diffuse" / f'{year}'
###############################################################################
def round_up_to_hundred(x):
    return np.ceil(x / 100) * 100
...
end_date = f'{year}-05-2'
dates = pd.date_range(start=start_date, end=end_date, freq='D')

# Load the GHI data from the NetCDF file
ds = xr.open_dataset(GLOB_datafile)
...
for date in dates:    # Generate the file paths for the current date
    ...
    output_file = output_plot_path / f'Beam_Diffuse_{file_date_str}_{pyrano_nr}pyrano_{method}.png'
    # plt.savefig(output_file, bbox_inches='tight')
    # plt.close()
